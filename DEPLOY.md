...
- ✅ Карточки звичок зі **змінюючимся кольорами** залежно від близькості до дати:
  ...
  - Помаранчевий (#f39c12) - завтра чи сьогодні
...
- ✅ Перегляд та обробка пропозицій користувачів
...
#### 💡 Система Пропозицій
- ✅ Плаваюча кнопка "Пропозиції" на Dashboard
- ✅ Користувачі можуть надіслати ідеї для покращення
- ✅ Адміни мають окремий таб для перегляду всіх пропозицій
- ✅ Статус пропозицій: "pending" або "read"
- ✅ Можливість додати коментар адміністратора
- ✅ Видалення опрацьованих пропозицій
...
- `POST /api/suggestions` - Подати нову пропозицію (auth required)
- `GET /api/suggestions` - Отримати всі пропозиції (admin only)
- `PUT /api/suggestion/:id` - Оновити статус та коментар (admin only)
- `DELETE /api/suggestion/:id` - Видалити пропозицію (admin only)

### Admin
- `GET /api/admin/users` - Список користувачів
- `POST /api/admin/block/:userId` - Заблокувати користувача
- `POST /api/admin/unblock/:userId` - Розблокувати користувача
- `GET /api/complaints` - Список скарг
- `POST /api/complaint` - Подати скаргу
- `PUT /api/complaint/:id` - Обробити скаргу
...
**Дата останнього оновлення**: 2026-05-02
**Версія**: 1.1.0 (Додана система пропозицій)
